...
var mag {m in MIESIACE, p in PRODUKTY} =  skladowanie[m] * skp[p, m] * <<progZmianyKosztowSkladowania; skladowaniePodProgiem, skladowanieNadProgiem>> x[p, m];
var f {s in SCENARIUSZE} = sum {m in MIESIACE, p in PRODUKTY} (kp[s,p,m]*x[p,m])+  sum {m in MIESIACE, p in PRODUKTY} (mag[m,p]) ;
